$(document).ready(function() {
	$.Juitter.start({
		...
		searchObject:"instantretro", // needed, you can insert a username here or a word to be searched for, if you wish multiple search, separate the words by comma.
		...
		live:"live-600", // the number after "live-" indicates the time in seconds to wait before request the Twitter API for updates.
		placeHolder:"twitter-updates", // Set a place holder DIV which will receive the list of tweets example <div id="juitterContainer"></div>
		loadMSG: "<span style='line-height:6px'><br /></span>Loading messages...", // Loading message, if you want to show an image, fill it with "image/gif" and go to the next variable to set which image you want to use on 
		imgName: "loader.gif", // Loading image, to enable it, go to the loadMSG var above and change it to "image/gif"
		total: 4, // number of tweets to be show - max 100
		readMore: "Read it on Twitter", // read more message to be show after the tweet content // removed due implementation to scopeq
	...
	$("#aRodrigo").click(function(){
		...
		$.Juitter.start({
			...
			live:"live-600" // it will be updated every 120 seconds/2 minutes
		});
	});
	$("#aIphone").click(function(){
		...
		$.Juitter.start({
			...
			live:"live-600"  // it will be update every 20 seconds 
		});
	});
	$("#aJuitter").click(function(){
		...
		$.Juitter.start({
			...
			live:"live-600" // it will be updated every 180 seconds/3 minutes
		});
	});
